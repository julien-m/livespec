...
function generateTests(features, dryRun) {
  ...
  if (!dryRun) {
    ...
    // Write audit report to .specs/
    if (generated > 0 && existsSync('.specs')) {
      const date = new Date().toISOString().slice(0, 10);
      const reportPath = `.specs/visual-scaffold-report-${date}.md`;
      const scaffolded = toGenerate.filter(f => !existsSync(f.testPath) === false).length;
      const reportLines = [
        `# Visual Scaffold Report — ${date}`,
        '',
        `**Generated:** ${generated} test file(s) | **Skipped:** ${skipped} (existing preserved) | **Baseline dirs:** ${dirsCreated}`,
        '',
        '## Features scaffolded',
        '',
        '| Feature | Test file | UI keywords detected |',
        '|---------|-----------|----------------------|',
        ...toGenerate.map(f =>
          `| \`${f.dir}\` | \`${f.testPath}\` | ${f.keywords.slice(0, 5).join(', ')} |`
        ),
        '',
        '## Features skipped (already have tests)',
        '',
        ...features.filter(f => f.hasTests).map(f => `- \`${f.dir}\``),
        '',
        '## Features skipped (no UI detected)',
        '',
        ...features.filter(f => !f.hasUI && !f.hasTests).map(f => `- \`${f.dir}\``),
        '',
        '*Generated by `scripts/migrate-visual-tests.js --generate`*',
      ];
