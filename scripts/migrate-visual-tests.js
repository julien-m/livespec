...
//
// Adaptive detection:
//   - If frontend/tests/e2e/ exists → generates tests there (Pencil mockup mode)
//   - If frontend/playwright.config.ts exists → updates that config
//   - If .specs/design/screens/ exists → uses Pencil mockups (current-*.png)
//   - Otherwise → legacy mode: tests/visual/ + playwright.visual.config.ts
...
const FRONTEND_MODE = hasFrontendE2E;
...
// In frontend/Pencil mode: baselines live in frontend/tests/e2e/baselines/{projectName}/
// In legacy mode: baselines/{dir}/{slug}/
const BASELINE_DIRS_LEGACY = ['mockups', 'fullpage', 'mobile', 'tablet', 'desktop', 'animations'];
...
export default defineConfig({
  ...
  snapshotPathTemplate: 'baselines/{projectName}/{arg}',
...
function generateE2ETemplate(feature, analysis = {}, externalSpecCtx = null) {
  ...
  // Relative path from frontend/tests/e2e/ to .specs/design/screens/
  const mockupRelPath = '../../.specs/design/screens';
...
import { fileURLToPath } from 'url';${fixturesImport}
...
const MOCKUP_DIR = path.join(__dirname, '${mockupRelPath}');
...
function printCompletionReport({ generated, skipped, configUpdated, scriptsAdded, featureList, routeGenerated = 0 }) {
  if (FRONTEND_MODE) {
    ...
    console.log('\n🎯 Next steps:\n');
    console.log('1. Review generated tests in frontend/tests/e2e/');
    ...
    console.log('   frontend/tests/e2e/baselines/{mobile,tablet,desktop,firefox,webkit}/\n');
    ...
    for (const f of featureList) {
      console.log(`  - tests/visual/${f} (3 test cases: mockup, fullpage, responsive)`);
    }
    if (skipped > 0) {
      console.log(`  - ${skipped} feature(s) skipped (existing tests preserved — AC-030)`);
    }
    ...
    console.log('\n🎯 Next steps:\n');
    console.log('1. Review generated tests in tests/visual/');
    console.log('   Verify the inferred ROUTE value and update it if needed.\n');
    console.log('2. (Optional) Add mockup baselines:');
    console.log('   Export Figma mockups to baselines/mockups/[feature]/main.png');
    ...
    console.log('   git add baselines/ tests/visual/ playwright.visual.config.ts package.json');
...
function generateTests(features, dryRun) {
  ...
  for (const feature of toGenerate) {
    ...
    if (dryRun) {
      ...
      if (!FRONTEND_MODE) {
        // Legacy mode: create baseline directories
        for (const dir of BASELINE_DIRS_LEGACY) {
...
function printScanTable(features) {
  ...
  const mockupLabel = PENCIL_MODE ? '.specs/design/screens/' : 'baselines/mockups/';
