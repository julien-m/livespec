...
La toute première action lors de `/spec-verify-output` est de poser le goal durable.
...
3. Rendre et sauvegarder le contrat dans un fichier de tâches :
   ...
   Le stdout affiche : `hash:<hash> | task-file:.specs/.runs/goal-spec-verify-output-<hash8>.md`
...
5. Émettre la commande slash `/goal` avec hash et référence au fichier :
   ```
   /goal hash:<hash> | spec-verify-output for <feature> — task list: .specs/.runs/goal-spec-verify-output-<hash8>.md
   ```
