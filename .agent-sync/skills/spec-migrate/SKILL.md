...
La toute première action lors de `/spec-migrate` est de poser le goal durable avec un contrat machine, puis de laisser `livespec goal prove` valider chaque tâche.
...
3. Rendre et sauvegarder le contrat immuable et l'état mutable :
   ...
   Le stdout affiche : `hash:<hash> | contract-file:$TMPDIR/livespec-goals/goal-spec-migrate-<hash8>.contract.json | state-file:$TMPDIR/livespec-goals/goal-spec-migrate-<hash8>.state.json`
...
5. Émettre la commande slash `/goal` avec hash et références machine :
   ```
   /goal hash:<hash> | spec-migrate for <feature> — contract-file:$TMPDIR/livespec-goals/goal-spec-migrate-<hash8>.contract.json — state-file:$TMPDIR/livespec-goals/goal-spec-migrate-<hash8>.state.json — mode:enforced
   ```
