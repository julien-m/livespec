...
## Verify
The set of command names in `commands/` must match exactly the set of `### /spec-*` headings in `.claude/rules/livespec-commands.md`. Command filenames in `commands/` carry the canonical `spec-` prefix (e.g. `commands/spec-test.md` ↔ heading `### /spec-test`).

Compare these two lists:

- Files: `find commands -maxdepth 1 -name '*.md' -not -name '*.expectations.md' | sed 's|.*/||;s|\.md$||' | sort`
- Headings: `grep -E '^### /spec-' .claude/rules/livespec-commands.md | sed 's|^### /||' | sort`
