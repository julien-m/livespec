...
- 2026-06-09: `$spec-fix` closed W49 pre-XCTest hang root cause: compiled journey runs now prefer shutdown iPhone/watch simulator devices over already-booted devices that may be wedged, while still emitting explicit `id=<UDID>` and boot/bootstatus checks before `xcodebuild test`.
- 2026-06-09: `$spec-fix` corrected the W51/W53 runner-ranking blockers and audit findings: simulator selection now ranks one cross-runtime candidate list by family, shutdown state, runtime rank, and device name, with UDID retained as a final deterministic code tie-breaker; already booted simulators skip `simctl boot` and still run `bootstatus`; regression coverage includes booted iPhone vs shutdown iPad, shutdown older iPhone vs booted newer iPhone, shutdown older Apple Watch vs booted newer Apple Watch, and invalid non-string `state`. Validation is scoped to the targeted runner, type, and LiveSpec checks plus V36 read-only verification: diff, targeted ruff/format, pytest, mypy, pyright, LiveSpec coherence, and LiveSpec doctor all passed because this worker prohibits native subagents.
