...
As a maintainer I need the exact Validator Tests workflow to execute all its existing checks against the repair SHA.
...
Scenario: Preserve complete CI proof
  ...
  And missing dependency or model credentials produce an explicit failure
Scenario: Missing private or model capability
  Given private dependency access or selected model credentials are unavailable
  When the existing CI validation requests that capability
...
flowchart TD
 ...
 D --> E[Existing generation selection and evaluation]
 E --> F{Required checks green on same SHA for 120 seconds?}
...
- **FR-005:** Retain existing generation selection and all required CI checks; record success only for the unchanged reviewed SHA after 120 seconds of stable successful checks. Maps to AC-005.
...
- Dependency revision: verified immutable Penflow Git commit; access stays private.
...
- Private-source or model credentials may be unavailable; no fallback can claim the missing proof.
...
- **SC-003:** No private source, secret, check exclusion or threshold reduction appears in the delivered diff (AC-001 through AC-005).
