...
- **Date:** 2026-06-27
- **Status:** Implemented
- **Input:** Clarify gate — an integrated pipeline phase that runs after specify and before plan, detects spec ambiguity (vague adjectives without measurable criteria, missing decisions), retains the full inventory and asks Impact × Uncertainty-ranked questions one at a time in presentation slices of up to 5, and writes accepted answers back under a dated `## Clarifications` section.
- **Feature Number:** 069
...
Read [requirement evidence integrity](../078-requirement-evidence-integrity/spec.md) for current clarification policy. FR-002/FR-003 detect French and English quality ambiguity using metrics associated with the same claim; an unrelated digit is insufficient. FR-005/FR-007 and SC-002 cap presentation only: retain every unresolved item, including critical items beyond the fifth. Merge grounded spec-review ambiguities, reuse cited approved decisions and deduplicate by requirement and observable decision. Current automatic progression blocks on every unresolved critical business interpretation. Accepted decisions preserve requirement IDs and source changes invalidate dependent readiness.
...
**Independent test:** Run a feature pipeline on a fixture spec that pairs one vague adjective without an applicable metric in its own clause with one unresolved placeholder marker; verify the gate runs after spec-review and before plan, and presents at least one question.
...
Feature: Clarify gate runs as an integrated phase between specify and plan
  ...
  Scenario: Spec with ambiguity produces a non-empty ranked question queue
    Given a spec.md that contains a vague quality adjective without a metric applicable to that quality in its own grammatical clause
    ...
    And each presentation slice contains at most 5 questions
    And all remaining opportunities stay in the inventory
```
...
flowchart TD
    ...
    C --> D[Rank complete inventory; present slice of at most 5]
    D --> E{Queue empty?}
    ...
    J -- Yes --> H
    J -- No --> K{Unresolved items outside this slice?}
    K -- Yes --> D
    K -- No --> L{Any unresolved critical item?}
    L -- Yes --> M[Block before plan]
    L -- No --> G
```
...
**Independent test:** Call `scan_clarification_opportunities` twice on the same fixture spec and assert the two opportunity lists are identical; assert "fast within 200 ms" resolves speed while "fast with 30-day retention" and "secure and fast within 200 ms" retain the unrelated unresolved quality.

#### Acceptance Scenarios (Gherkin — source of truth for tests)

```gherkin
Feature: Deterministic ambiguity detection across three categories
  Scenario: Vague adjective without its own measurable criterion is flagged
  ...
  Scenario: A metric in the same quality clause resolves that quality
    Given a sentence "the export must be fast within 200 ms"
    When the gate scans the line
    Then it records no vague-adjective opportunity for "fast"

  Scenario: An unrelated number cannot resolve speed
    Given a sentence "the export must be fast with 30-day retention"
    When the gate scans the line
    Then it records a non-functional quality opportunity for "fast"

  Scenario: A speed metric cannot resolve security in another clause
    Given a sentence "the export must be secure and fast within 200 ms"
    When the gate scans the line
    Then it records an opportunity for "secure"
    And it records no opportunity for "fast"
  ...
  Scenario: Requirement-ID digits do not count as a numeric criterion
    ...
    When the gate scans the line
    Then the digits in FR-001, AC-002, and SC-001 are excluded from the quality-metric check
...
flowchart TD
    ...
    F --> I{Vague adjective present?}
    I -- Yes --> G{Own clause has metric for this quality?}
    G -- Yes --> H[No opportunity for this adjective]
    G -- No --> J[Add non-functional quality opportunity]
...
**As a** developer answering the gate, **I want** questions asked one at a time in ranked order, with at most 5 per presentation slice and the rest retained, with my accepted answers written back into the spec under a dated `## Clarifications` section, **so that** the resolution is bounded and traceable in the spec itself.
...
**Independent test:** Provide a spec that scans to 7 opportunities; verify the first slice offers 5 questions and retains the other 2, including any critical question that still blocks progression; accept 2 answers and verify a `## Clarifications` section with a `### Session YYYY-MM-DD` subheading and 2 `- Q: … -> A: …` bullets appears, and the affected requirement text is updated in place.

#### Acceptance Scenarios (Gherkin — source of truth for tests)

```gherkin
Feature: Bounded questioning with persisted clarifications
  Scenario: Presentation is capped but the inventory is complete
    Given a spec that scans to 7 clarification opportunities
    When the gate ranks the complete inventory
    Then the first presentation slice offers 5 questions in ranked order
    And the other 2 remain available for a later slice

  Scenario: A critical question beyond the first slice still blocks
    Given 7 unresolved questions with a critical item outside the first slice
    When the first 5 questions are resolved
    Then the remaining inventory still contains the critical item
    And the next slice presents the remaining questions in ranked order
    And progression stays blocked until the critical item is resolved
...
```mermaid
flowchart TD
    A[Complete ranked inventory; slice of at most 5] --> B{Queue empty?}
    ...
    H -- Yes --> D
    H -- No --> I{Items remain outside slice?}
    I -- Yes --> A
    I -- No --> J{Any unresolved critical item?}
    J -- Yes --> K[Block before plan]
    J -- No --> Z
```
...
| AC-002 | `scan_clarification_opportunities(spec_path)` returns an opportunity for each vague quality adjective whose own grammatical clause lacks a metric applicable to that quality; unrelated numbers and metrics for another quality do not resolve it | P1 | Story 2 |
...
| AC-005 | `rank_clarification_opportunities` orders by descending Impact × Uncertainty with stable tie-breaks; `limit=None` returns the complete inventory, while numeric limits expose at most 5 items per presentation slice without discarding the remaining inventory | P1 | Story 3 |
...
| AC-007 | Unresolved questions are presented one at a time in ranked order, at most 5 per presentation slice; all remaining questions are retained and every unresolved critical item blocks progression | P2 | Story 3 |
...
| AC-011 | In `--auto` mode the gate accepts only deterministic recommendations grounded in the constitution or existing spec text; for a question needing a human decision it emits `BLOCKED at step 1.6 - decision_needed - clarify question requires human answer` in the `/spec-feature` Phase 1.6 context, and instead leaves an explicit `[ASSUMED]` note in the `/spec-specify` Step 5.9 context; that drafting note does not resolve a critical item or authorize mandatory progression | P2 | Story 4 |
...
**Criterion:** `scan_clarification_opportunities(spec_path)` returns an opportunity for each vague quality adjective whose own grammatical clause lacks a metric applicable to that quality; unrelated numbers and metrics for another quality do not resolve it
...
**Criterion:** `rank_clarification_opportunities` orders by descending Impact × Uncertainty with stable tie-breaks; `limit=None` returns the complete inventory, while numeric limits expose at most 5 items per presentation slice without discarding the remaining inventory
...
**Criterion:** Unresolved questions are presented one at a time in ranked order, at most 5 per presentation slice; all remaining questions are retained and every unresolved critical item blocks progression
...
**Criterion:** In `--auto` mode the gate accepts only deterministic recommendations grounded in the constitution or existing spec text; for a question needing a human decision it emits `BLOCKED at step 1.6 - decision_needed - clarify question requires human answer` in the `/spec-feature` Phase 1.6 context, and instead leaves an explicit `[ASSUMED]` note in the `/spec-specify` Step 5.9 context; that drafting note does not resolve a critical item or authorize mandatory progression
...
| FR-002 | The system must detect each French or English vague quality adjective in `VAGUE_ADJECTIVES` when its own grammatical clause lacks a metric applicable to that quality; unrelated numeric tokens or metrics for another quality do not resolve it | AC-002 |
| FR-003 | The system must exclude digits glued to letters and requirement IDs (`FR-`, `AC-`, `SC-`) from the quality-metric check so they do not silence a genuine vague-adjective opportunity | AC-003 |
...
| FR-005 | The system must score each opportunity as Impact × Uncertainty, order opportunities by descending score with a stable tie-break, and cap only the presented queue at 5 while retaining the complete unresolved inventory | AC-005, AC-007 |
...
| FR-007 | The system must present unresolved questions one at a time in ranked order, expose at most 5 per presentation slice, retain all remaining questions, and prevent progression while any critical item remains unresolved | AC-007 |
...
**Requirement:** The system must detect each French or English vague quality adjective in `VAGUE_ADJECTIVES` when its own grammatical clause lacks a metric applicable to that quality; unrelated numeric tokens or metrics for another quality do not resolve it
**AC References:** [AC-002](#ac-002)

### FR-003

**Requirement:** The system must exclude digits glued to letters and requirement IDs (`FR-`, `AC-`, `SC-`) from the quality-metric check so they do not silence a genuine vague-adjective opportunity
...
**Requirement:** The system must score each opportunity as Impact × Uncertainty, order opportunities by descending score with a stable tie-break, and cap only the presented queue at 5 while retaining the complete unresolved inventory
...
**Requirement:** The system must present unresolved questions one at a time in ranked order, expose at most 5 per presentation slice, retain all remaining questions, and prevent progression while any critical item remains unresolved
...
**Requirement:** In auto mode the system must accept only deterministic recommendations grounded in the constitution or existing spec text; for a question needing a human decision it emits the canonical `BLOCKED at step 1.6 - decision_needed - clarify question requires human answer` line in the `/spec-feature` Phase 1.6 context, and instead leaves an explicit `[ASSUMED]` note in the `/spec-specify` Step 5.9 context; that drafting note does not resolve a critical item or authorize mandatory progression
...
|---|---|---|
| ClarifyOpportunity | An immutable clarification candidate grounded in source evidence; identity and resolution are supplied by scanners/review consumers, which own validation. Read the [dataclass](../../../validator/clarify_gate.py), [inventory](../../../validator/clarify_inventory.py) and [decision ledger](../../../validator/clarify_decisions.py). | category, question, impact, uncertainty, evidence_path, evidence_line, evidence_text, requirement_id (feature-qualified `<feature-slug>:<FR/AC/SC-ID>`, empty for unscoped prose), decision_key (observable decision within that requirement), critical (unresolved item blocks readiness), source_hash (source fingerprint), resolution (accepted answer; empty while pending), resolution_provenance (supporting citation tuple), score (computed property: impact × uncertainty) |
| ClarificationQueue | The complete ranked inventory with bounded presentation slices (≤ 5) — conceptual, implemented as a plain ordered `list[ClarifyOpportunity]`, not a named type | ordered opportunities, limit |
...
- **More than 5 opportunities:** The full ranked inventory retains all opportunities; each slice presents at most 5 and later slices expose remaining items. A critical item outside the current slice still blocks progression (AC-005, AC-007).
...
- **Metric for this quality:** Only a metric applicable to the adjective in its own grammatical clause resolves it. Retention counts do not quantify speed, and a speed metric does not resolve security (AC-002).
...
| SC-002 | Each presentation slice contains at most 5 questions while the complete inventory remains available | Verify the default slice bound, exhaustive `limit=None` inventory and blocking critical items beyond the first slice |
