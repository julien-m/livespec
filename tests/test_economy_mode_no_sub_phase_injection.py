...
The `--economy` mode in `commands/feature.md` runs specify/plan/implement
...
FEATURE_MD = Path(__file__).parent.parent / "commands" / "feature.md"
...
def test_economy_paragraphs_present() -> None:
    ...
    assert paragraphs, "no economy mode blockquote found in commands/feature.md"
...
def test_economy_paragraphs_do_not_prepend_subcmd_headers() -> None:
    for paragraph in _economy_blockquote_sections():
        # A `/spec.specify` mention inside the economy paragraph would be a
        # synthetic header — economy mode does not spawn subagents so it
        # should not synthesise these.
        assert "/spec.specify" not in paragraph, paragraph
        assert "/spec.plan" not in paragraph, paragraph
        assert "/spec.implement" not in paragraph, paragraph
